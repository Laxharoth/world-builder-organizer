...
export function parseVariable(name: string) {
    while (regexFindVariable.test(name)) {
        regexFindVariable.exec(name)?.forEach(str => {
...
export function extractContentVariables(content: string) {
    ...
    while (regexFindVariable.test(content)) {
        regexFindVariable.exec(content)?.forEach(str => {
...
export async function setVariable(
    name: string, placeHolder: string|null = null, value: string|null = null,
    ...
    value =
        await vscode.window.showInputBox({placeHolder, value, prompt}) || null;
    if (value) {
